...
describe("extension messages", () => {
  it("loads panel copy by language", () => {
    expect(translate("en-us", "create_room_button")).toBe("Create");
    expect(translate("zh-cn", "create_room_button")).toBe("创建");
  });

  it("falls back for unsupported languages", () => {
    expect(getMessages("fr-FR").join_room_button).toBe("Join");
  });

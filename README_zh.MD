...
运行 `python script/build_extension.py` 后，在 Chrome 扩展页面加载 `source/chrome`。
...
此外还有一个 Chrome 壳插件，代码在 `source/extension/extension.js`

代码中 {{{ }}} 代表的是将某个文件插入到此处

输入命令 `python script/build_extension.py` 能够进行编译，编译结果会写入 `source/chrome`
